...
class _DiabetesPredictionPageState extends State<DiabetesPredictionPage> {
  ...
  bool isGroundTruthSwitched =
      false; // Track the state of the ground truth switch
  ...
  void updateFieldsWithSampleData(int sampleIndex) {
    ...
    diabetesPedigreeFunctionController.text =
        sample['DiabetesPedigreeFunction'].toString();
  ...
  Future<void> storePredictionInFirestore() async {
    try {
      ...
      CollectionReference predictionsCollection =
          usersCollection.doc('predictions').collection('predictions');
      ...
      await predictionsCollection.add({
        ...
        'GroundTruth':
            isGroundTruthSwitched ? getGroundTruth(selectedSample) : '-',
  ...
  String getGroundTruth(int sampleIndex) {
    if (sampleData.containsKey(sampleIndex)) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: Color.fromARGB(255, 252, 0, 189),
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            buildParameterTextField('Diabetes Pedigree Function',
  ...
  Widget buildParameterTextField(
    ...
    return TextField(
      ...
      style: TextStyle(fontSize: 16),
      enabled: !isGroundTruthSwitched, // Disable input when switch is enabled
      onChanged: (value) {
        if (isGroundTruthSwitched) {
          // Show Snackbar when user tries to enter data with ground truth enabled
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Cannot enter data with Ground Truth enabled'),
            ),
          );
          // Clear the text field when trying to input with ground truth enabled
          controller.clear();
        }
      },
    );
